...
export class MessageAnalyzer {
  ...
  async analyzeMessage(message: DiscordMessage): Promise<QuestionAnalysis> {
    ...
    const questionAnalysis = await this.fireworks.analyzeMessage(
      message.content,
    )
    ...
    if (questionAnalysis.isQuestion && questionAnalysis.confidence >= 70) {
      try {
        // Get related messages using enhanced relevance algorithm
        ...
        if (contextMessages.length > 0) {
          ...
          analysis.suggestedAnswer =
            'No previous context available to generate answer.'
        }
