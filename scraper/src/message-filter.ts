...
export class MessageFilter {
  ...
  constructor(trivialPhrases: string[] = [], minLength: number = 3) {
    // Default trivial phrases
    const defaultTrivial = [
      'hi', 'hello', 'hey', 'sup', 'yo',
      'good morning', 'good afternoon', 'good evening', 'good night',
      'gm', 'gn', 'ty', 'thanks', 'thx',
      'lol', 'lmao', 'haha', 'ok', 'okay',
      'yes', 'no', 'yep', 'nope', 'sure',
      'brb', 'gtg', 'ttyl', 'afk',
      '+1', '-1', '👍', '👎', '❤️', '🔥',
      'same', 'agreed', 'this', 'exactly',
      'nice', 'cool', 'awesome', 'great'
    ];

    this.trivialPhrases = new Set([...defaultTrivial, ...trivialPhrases.map(p => p.toLowerCase())]);
  ...
  isTrivial(message: DiscordMessage): boolean {
    ...
    if (withoutMentions.length === 0 || this.trivialPhrases.has(withoutMentions)) {
      return true;
    }
  ...
  isFromBot(message: DiscordMessage): boolean {
    ...
    return message.rawData?.author?.bot === true || 
           message.authorName.toLowerCase().includes('bot') ||
           message.authorId.endsWith('0000'); // Discord bot IDs often end in 0000
  }

  shouldProcess(message: DiscordMessage, options: {
    ignoreBots?: boolean;
    ...
    if (requiredKeywords.length > 0 && !this.hasKeywords(message, requiredKeywords)) {
      return false;
    }
